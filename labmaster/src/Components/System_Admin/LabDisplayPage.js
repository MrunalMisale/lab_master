...
function LabDisplayPage() {
    ...
    async function fetchData(){
        let result = await fetch("<--firebase realtime database endpoint address-->sysadmin/configure.json",{
            method:"get"
        })
    ...
    function onConfigureLabHandler(labno){
        navigate(`/system_admin/dashboard/labs/configure/${labno}`)
    }
                        ...
                        <tbody key={id}>
                            <tr key={id} className="flex justify-between ml-1 my-1 border border-slate-300 shadow-md shadow-slate-300">
                                ...
                                <td>
                                    ...
                                    onClick={()=>onConfigureLabHandler(x.lab_no)}></img>
                                </td>
                            </tr>
                        </tbody>
                    )}
